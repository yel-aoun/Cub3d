...
/*   Updated: 2023/02/09 10:56:20 by yel-aoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_textures_no(t_data *data, double py, double px, t_add *add)
{
	...
	clr = add->n.addr + (y * add->n.l_length + x * (4));
	return (clr);
}
...
int	ft_textures(t_data *data, double py, double px, t_add *add)
{
	...
	if (data->state == 0)
	{
		...
		clr = add->w.addr + (y * add->w.l_length + x * (4));
	}
	else if (data->state == 1)
	{
		x = (int)(px * add->e.img_w) % add->e.img_w;
		y = (int)(py * add->e.img_h) % add->e.img_h;
		clr = add->e.addr + (y * add->e.l_length + x * (4));
	}
	...
	else if (data->state == 3)
	{
		...
		clr = add->s.addr + (y * add->s.l_length + x * (4));
	}
